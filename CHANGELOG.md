...
- [FLEET-NET] `HeartbeatData` gains two new fleet-summed network rate fields: `net_rx_bytes_sec` and `net_tx_bytes_sec` (uint64, bytes/sec, `omitempty`). The agent computes deltas between consecutive heartbeats across all non-loopback interfaces (physical NICs, bridges, virtual NICs) and reports the rates. Loopback interfaces (`lo`, `lo0`, `Loopback*`) are excluded. Rates are clamped to 0 on the first heartbeat after agent start, on counter rollover, and on system clock jumps backwards. Old agents continue to function — fields are `omitempty`, and old servers receiving the new fields silently ignore unknown JSON keys. The dashboard's per-device cards gain a second sub-chart (100px tall, bytes/sec Y-axis with auto-scaling B/s → KB/s → MB/s → GB/s) showing **In** (cyan) and **Out** (orange) lines.
- [FLEET-NET] Frontend `useFleetMetrics` hardens against malformed heartbeat payloads: any `net_rx_bytes_sec` / `net_tx_bytes_sec` that arrives as `NaN`, `±Infinity`, or a negative number is clamped to 0 before reaching recharts. This protects the dashboard from a buggy or malicious agent posting non-finite values that would otherwise corrupt the chart's axis range computation.
