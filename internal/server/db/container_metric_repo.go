...
func (r *ContainerMetricRepo) StoreBatch(ctx context.Context, deviceID string, metrics []models.ContainerMetric) error {
	...
	for i, m := range metrics {
		rows[i] = []interface{}{deviceID, m.ContainerName, m.ContainerID, m.Timestamp, m.CPUPercent, m.MemUsage, m.MemLimit, m.CPULimit}
	}
	_, err := r.db.Pool.CopyFrom(
		ctx,
		pgx.Identifier{"container_metrics"},
		[]string{"device_id", "container_name", "container_id", "timestamp", "cpu_percent", "mem_usage", "mem_limit", "cpu_limit"},
