...
func (r *DeviceRepo) GetByID(ctx context.Context, id string) (*models.Device, error) {
	...
	err := r.db.Pool.QueryRow(ctx,
		`SELECT id, short_id, hostname, arch, agent_version, primary_ip, status, location, tags, docker_available, docker_container_count, docker_group_count, auto_patch, hardware_profile,
		 last_heartbeat, last_telemetry, created_at, updated_at FROM devices WHERE id=$1`, id,
	).Scan(&d.ID, &d.ShortID, &d.Hostname, &d.Arch, &d.AgentVersion, &d.PrimaryIP, &d.Status, &d.Location, &tagsJSON, &d.DockerAvailable, &d.DockerContainerCount, &d.DockerGroupCount, &d.AutoPatch, &hwJSON,
...
func (r *DeviceRepo) List(ctx context.Context) ([]models.Device, error) {
	...
	for rows.Next() {
		var d models.Device
		var tagsJSON, hwJSON []byte
		if err := rows.Scan(&d.ID, &d.ShortID, &d.Hostname, &d.Arch, &d.AgentVersion, &d.PrimaryIP, &d.Status, &d.Location, &tagsJSON, &d.DockerAvailable, &d.DockerContainerCount, &d.DockerGroupCount, &d.AutoPatch, &hwJSON,
...
func (r *DeviceRepo) UpdateDockerAvailable(ctx context.Context, id string, available bool, containerCount, groupCount int) error {
	_, err := r.db.Pool.Exec(ctx,
		`UPDATE devices SET docker_available=$2, docker_container_count=$3, docker_group_count=$4, updated_at=NOW() WHERE id=$1`,
		id, available, containerCount, groupCount)
	return err
}
...
func (r *DeviceRepo) ListByVersion(ctx context.Context, version string) ([]models.Device, error) {
	query := `SELECT id, short_id, hostname, arch, agent_version, primary_ip, status, location, tags, docker_available, docker_container_count, docker_group_count, auto_patch, hardware_profile,
