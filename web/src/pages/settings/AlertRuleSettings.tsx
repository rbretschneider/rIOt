...
const METRICS = [
  ...
  { value: 'failed_logins_interval', label: 'Auth Failures (per interval)' },
...
const METRIC_DEFAULTS: Record<string, { operator: string; threshold: number; severity: string; cooldown: number; hint: string }> = {
  ...
  failed_logins_interval: { operator: '>', threshold: 0, severity: 'warning', cooldown: 300, hint: 'SSH/sudo/console auth failures since last telemetry push (Linux only). Raise the threshold on internet-facing hosts to avoid notification floods.' },
